...
namespace BroMakerLib.Unlocks
{
    internal static class BroUnlockManager
    {
        ...
        private static readonly List<Action<string, StoredHero>> broUnlockedCallbacks = new List<Action<string, StoredHero>>();
        ...
        internal static void UnlockBro(BroUnlockState state, bool queueUnlock = true)
        {
            ...
            // Notify callbacks
            foreach (var callback in broUnlockedCallbacks)
            {
                callback.Invoke(state.BroName, bro);
            }
        }
        ...
        internal static void RegisterNotifyBroUnlocked(Action<string, StoredHero> callback)
        {
            broUnlockedCallbacks.Add(callback);
        }
        #endregion
    }
}
