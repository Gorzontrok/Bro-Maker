...
{
    [HeroPreset("TrentBroser", HeroType.TrentBroser)]
    public class TrentBroserM : TrentBroser, ICustomHero, IAbilityOwner
    {
        ...
        {
            var prefab = HeroController.GetHeroPrefab(HeroType.TrentBroser);
            if (prefab == null) return;
            CopySerializedValues(prefab);
            if (faderSpritePrefab == null) faderSpritePrefab = prefab.faderSpritePrefab;
            if (disarmedGunMaterial == null) disarmedGunMaterial = prefab.disarmedGunMaterial;
            if (parachute == null) parachute = prefab.parachute;
        }
        #endregion
        ...
        protected override void Awake()
        {
            try
            {
                ...
                var awakePrefab = HeroController.GetHeroPrefab(HeroType.TrentBroser).As<TrentBroser>();
                if (awakePrefab != null)
                {
